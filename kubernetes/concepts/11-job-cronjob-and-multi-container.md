...
Deployment는 "계속 떠 있는 서비스"를 위한 워크로드입니다. 이 문서는 그 밖의 실행 형태를 다룹니다 — 한 번 실행하고 끝나는 작업(Job), 주기적으로 실행되는 작업(CronJob), 그리고 Pod 하나에 컨테이너를 여러 개 두는 패턴(init, sidecar)입니다.

> **CKAD 시험 범위** — Application Design and Build(20%) 도메인의 핵심 출제 영역입니다. 워크로드 리소스 선택과 멀티 컨테이너 패턴이 그대로 시험 문제로 나옵니다.
