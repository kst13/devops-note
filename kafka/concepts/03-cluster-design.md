...
실서버 3대에 Kafka를 올려 **MSA 서비스 간 비동기 메시징** 백본으로 쓰는 상황을 기준으로, 어떤 토폴로지·복제·보안 설정을 선택해야 하는지와 그 이유를 정리합니다. 실제 설치 절차는 [KRaft 3노드 클러스터 설치 및 설정 방법](04-kraft-cluster-installation.md)에서 다룹니다.
...
- 데이터는 host의 **전용 디스크**(`/data/kafka`)에 저장하고 컨테이너에 bind mount 합니다. 컨테이너를 재생성해도 로그가 보존됩니다. Kafka는 순차 쓰기가 많으므로 가능하면 별도 디스크를 씁니다.
